...
function getDiagnostico() {
	const profundidadSondajeMedia = $("#suma4").text();
	const nivelInsercionMedia = $("#suma5").text();
	const porcentajeSangrado = $("#suma").text();
	const porcentajePlaca = $("#suma2").text();

	let diagnostico = '';

	// Evaluar el diagnóstico en base a los valores calculados
	if (profundidadSondajeMedia <= 3 && nivelInsercionMedia <= 3 && porcentajeSangrado === '0') {
		diagnostico = 'Salud periodontal';
	} else if (profundidadSondajeMedia <= 3 && nivelInsercionMedia <= 3 && porcentajeSangrado > '0') {
		diagnostico = 'Gingivitis';
	} else {
		diagnostico = 'Periodontitis';

		if (profundidadSondajeMedia >= 4 && profundidadSondajeMedia <= 5) {
			diagnostico += ' inicial';
		} else if (profundidadSondajeMedia >= 6 && profundidadSondajeMedia <= 7) {
			diagnostico += ' moderada';
		} else if (profundidadSondajeMedia >= 8) {
			diagnostico += ' avanzada';
		}
	}

	// Actualizar el campo de diagnóstico en el formulario
	$("#diagnostico").val(diagnostico);

	// Desmarcar las casillas de verificación existentes
	$('input[type="checkbox"]').prop('checked', false);

	// Marcar las casillas de verificación correspondientes al diagnóstico
	if (diagnostico.includes('Salud periodontal')) {
		$("#saludable").prop('checked', true);
	} else if (diagnostico.includes('Gingivitis')) {
		$("#gingivitis").prop('checked', true);
	} else if (diagnostico.includes('Periodontitis')) {
		$("#periodontitis").prop('checked', true);
		if (diagnostico.includes('inicial')) {
			$("#inicial").prop('checked', true);
		} else if (diagnostico.includes('moderada')) {
			$("#moderada").prop('checked', true);
		} else if (diagnostico.includes('avanzada')) {
			$("#avanzada").prop('checked', true);
		}
	}
	return diagnostico;
}
